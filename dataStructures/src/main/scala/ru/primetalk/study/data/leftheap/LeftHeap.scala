...
given HeapImpl: Heap[LeftHeap] with
  def merge[T: Ordering](a: LeftHeap[T], b: LeftHeap[T]): LeftHeap[T] = (a, b) match {
  ...
  def toListDesc[T: Ordering](h: LeftHeap[T], res: List[T] = Nil): List[T] =
    h match {
      case LeftHeap.Empty => res
      case LeftHeap.LeftHeap1(_, elem, a, b) => toListDesc(HeapImpl.merge(a, b), elem :: res)
    }
